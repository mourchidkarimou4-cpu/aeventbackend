...
class Pack(models.Model):
    ...
    promo_fin  = models.DateField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            slug = base
            n = 1
            while Pack.objects.filter(slug=slug).exclude(pk=self.pk).exists():
                slug = f"{base}-{n}"
                n += 1
            self.slug = slug
        super().save(*args, **kwargs)
...
class PackItem(models.Model):
    ...
    def __str__(self):
        return f"{self.quantite}x {self.nom_item} ({self.pack.nom})"
